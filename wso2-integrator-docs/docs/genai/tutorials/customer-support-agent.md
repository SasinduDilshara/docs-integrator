---
sidebar_position: 1
title: "Build an AI-Powered Customer Support Agent"
description: Create a customer support agent with order lookup, FAQ retrieval via RAG, and automated ticket creation in WSO2 Integrator.
---
...
You will build an intelligent customer support agent that handles order inquiries, answers FAQs from a knowledge base using RAG, and creates support tickets when issues need escalation. This tutorial combines agent tool binding, retrieval-augmented generation, and external service integration into a single, production-ready integration.

## What You'll Build

A customer support agent with three capabilities:
...
- How to define multiple tools and bind them to a single agent
- How to integrate RAG-based retrieval as a tool
- How to configure system prompts for customer support scenarios
- How to add tracing to observe agent behavior in real time
...
:::info
This tutorial uses OpenAI as the model provider. You can substitute any supported provider — see [AI & LLM Connectors](/docs/connectors/ai-llms) for connection configuration details.
:::
...
3. Name the project `customer-support-agent` and choose a directory.
...
1. Click **+ Add Artifact** and select **Chat Agent**.
2. Name the agent `SupportAgent`.

Add the order lookup tool to your project:
...
:::info
This tutorial uses an in-memory vector store for simplicity. For production deployments, connect to Pinecone, Weaviate, or pgvector. See [Vector Database Connectivity](/docs/genai/rag/vector-database).
:::
...
Wire all three tools together with a system prompt that guides the agent's behavior:
...
- **Add conversation logging** — Persist conversations for quality review using [Conversation Logging](/docs/genai/observability/conversation-logging).
...
- [Agent Architecture & Concepts](/docs/genai/agents/architecture-concepts) — Understand the agent loop and reasoning process
- [Input / Output Guardrails](/docs/genai/guardrails/input-output-guardrails) — Add safety checks to validate agent inputs and outputs
