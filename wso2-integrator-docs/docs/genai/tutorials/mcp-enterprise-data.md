---
sidebar_position: 4
title: "Expose Enterprise Data via MCP"
description: Build an MCP server that exposes database and API data as tools any LLM can discover and call from WSO2 Integrator.
---

# Expose Enterprise Data via MCP

**Time:** 30-45 minutes

You will build an MCP (Model Context Protocol) server that exposes your enterprise database and APIs as tools that any MCP-compatible AI assistant can discover and call. This approach turns your existing data sources into a universal tool layer — connect once in WSO2 Integrator, and every LLM client in your organization can query customers, look up orders, and get product information through a standardized protocol.
...
- How to create an MCP server in WSO2 Integrator
- How to expose database queries as MCP tools
- How to add input validation and security to MCP tools
- How to test with the MCP Inspector and connect from an external AI assistant

## Prerequisites

- [WSO2 Integrator VS Code extension installed](/docs/get-started/install)
- A running database (this tutorial uses MySQL; any supported database works)
- Familiarity with [MCP Overview](/docs/genai/mcp/overview)

:::info
This tutorial connects to a MySQL database. WSO2 Integrator supports all major databases — see [Database Connectors](/docs/connectors/databases) for connection configuration.
:::
...
3. Name the project `mcp-enterprise-data` and choose a directory.
...
:::warning
Always secure MCP servers that expose enterprise data. Use API keys at minimum, and consider OAuth2 for multi-tenant deployments. See [MCP Security & Authentication](/docs/genai/mcp/security-authentication) for production guidance.
:::
...
- **Add audit logging** — Log every tool invocation for compliance. See [Conversation Logging](/docs/genai/observability/conversation-logging).
...
- [Exposing Integrations as MCP Servers](/docs/genai/mcp/exposing-mcp-servers) — Deep dive into MCP server configuration
- [Consuming MCP Tools](/docs/genai/mcp/consuming-mcp-tools) — Connect your agents to external MCP servers
- [MCP Security & Authentication](/docs/genai/mcp/security-authentication) — Production security patterns for MCP
