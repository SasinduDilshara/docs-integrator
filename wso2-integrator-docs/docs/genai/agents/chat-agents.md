...
In this guide, you will build a **Math Tutor** chat agent step by step. By the end, you will have a working agent that can perform arithmetic operations, hold multi-turn conversations, and produce traceable responses — all without writing boilerplate HTTP or session-management code.
...
:::info
The default in-memory implementation stores conversation history for the duration of the running session. When the service restarts, history is cleared. For persistent or semantic memory options, see [Configure Agent Memory](memory-configuration.md).
:::
...
- [Bind Tools to Agents](tool-binding.md) — Connect your agent to external services, connectors, and APIs
- [Configure Agent Memory](memory-configuration.md) — Choose between conversation-window, semantic, and persistent memory strategies
- [Expose an Agent as an API](api-exposed-agents.md) — Serve your agent as a programmatic HTTP/REST endpoint for external consumers
